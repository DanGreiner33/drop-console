...
const STATUS_COLOR = { idle: T.dim, warming: T.warn, armed: T.info, live: T.accent, cooldown: T.warn, done: T.ok, failed: T.bad }
...
function Tabs({ tab, onChange }) {
  ...
  return (
    <nav style={{ display: 'flex', borderBottom: `1px solid ${T.border}`, background: '#080808', padding: '0 16px' }}>
      {tabs.map(([k, label]) => (
        <button key={k} onClick={() => onChange(k)} style={{ background: 'transparent', border: 'none', borderBottom: `2px solid ${tab === k ? T.accent : 'transparent'}`, color: tab === k ? T.accent : T.dim, padding: '12px 18px', fontSize: 11, letterSpacing: 2, textTransform: 'uppercase', cursor: 'pointer', fontFamily: 'inherit' }}>{label}</button>
      ))}
    </nav>
  )
}
  ...
  return (
    <>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr) auto', gap: 10, alignItems: 'end' }}>
          ...
          <Field label="priority">
            <select style={inputStyle} value={draft.priority} onChange={e => setDraft({ ...draft, priority: e.target.value })}>
              <option value="p0">p0 critical</option><option value="p1">p1 high</option><option value="p2">p2 normal</option><option value="p3">p3 low</option>
            </select>
          </Field>
          <Field label="starts at"><input style={inputStyle} value={draft.startsAt} onChange={e => setDraft({ ...draft, startsAt: e.target.value })} placeholder="2026-04-30T14:00:00Z" /></Field>
      ...
      <Panel title={`drops (${drops.length})`} right={<Pill color={T.dim}>click status to advance</Pill>}>
        {drops.length === 0 ? (
          <div style={{ padding: '24px 0', textAlign: 'center', color: T.dim, fontSize: 11, letterSpacing: 2, textTransform: 'uppercase' }}>no drops armed</div>
        ) : (
          <div style={{ display: 'grid', gap: 6 }}>
            ...
            {drops.map(d => (
              <div key={d.id} style={{ display: 'grid', gridTemplateColumns: '60px 1fr 160px 110px 110px 110px 90px', gap: 8, alignItems: 'center', padding: '8px', border: `1px solid ${T.border}`, borderRadius: 2, fontSize: 12 }}>
                ...
                <div><div style={{ color: T.text }}>{d.name}</div><div style={{ color: T.dim, fontSize: 10 }}>{d.id}</div></div>
                ...
                <div style={{ display: 'flex', gap: 4 }}><Btn danger onClick={() => onFail(d.id)}>x</Btn><Btn onClick={() => onRemove(d.id)}>rm</Btn></div>
              </div>
            ))}
          </div>
        )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 16 }}>
        <Field label="proxy pool">
          <select style={inputStyle} value={stack.proxyPool} onChange={e => onUpdate({ proxyPool: e.target.value })}>
            <option value="residential-tier1">residential-tier1</option><option value="residential-tier2">residential-tier2</option><option value="isp-static">isp-static</option><option value="datacenter">datacenter</option>
          </select>
        </Field>
        <Field label="captcha provider">
          <select style={inputStyle} value={stack.captchaProvider} onChange={e => onUpdate({ captchaProvider: e.target.value })}>
            <option value="capsolver">capsolver</option><option value="2captcha">2captcha</option><option value="anti-captcha">anti-captcha</option><option value="none">none</option>
          </select>
        </Field>
        <Field label="monitor">
          <select style={inputStyle} value={stack.monitor} onChange={e => onUpdate({ monitor: e.target.value })}>
            <option value="sole-retriever">sole-retriever</option><option value="hydra">hydra</option><option value="custom-webhook">custom-webhook</option>
          </select>
        </Field>
        <Field label="concurrency"><input style={inputStyle} type="number" min="1" max="256" value={stack.concurrency} onChange={e => onUpdate({ concurrency: Number(e.target.value) || 1 })} /></Field>
        <Field label="retries"><input style={inputStyle} type="number" min="0" max="10" value={stack.retries} onChange={e => onUpdate({ retries: Number(e.target.value) || 0 })} /></Field>
      </div>
    </Panel>
  )
}
